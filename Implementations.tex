...
El cliente se comunica con un nodo mediante el protocolo HTTP.
%
...
%
De los distintos clientes nativos que proveía Tendermint, el cliente HTTP
es la implementación más razonable de utilizar en un entorno
...
hace a través del módulo \textit{crypto} de la librería Golang
del protocolo Ethereum~\footnote{Dispnible en \url{https://pkg.go.dev/github.com/ethereum/go-ethereum/crypto}.}.
%
Esta librería trabaja con el algoritmo de firma digital ECDSA, uno de los esquemas de firmas más
...
La función \texttt{Ecrecover} es la función de recuperación de clave pública que facilita la librería utilizada.
...
\begin{figure}
  ...
  \subfloat[Se agrega la nuevas épocas a la \setchain]{\includegraphics[width = 3.5in]{figures/compresschain-flow-6.png}\label{subfig:compress-flow-g}} 
...
\subsection{Algoritmo distribuido para la inversión de hashes}
...
\begin{figure}
  ...
  \subfloat[La aplicación requiere la inversión de un hash]{\includegraphics[width = 3.5in]{figures/hash-reversal-1.png}\label{subfig:hash-flow-a}}\\
  \subfloat[El \hcollector conoce el hash]{\includegraphics[width = 3.5in]{figures/hash-reversal-2.png}\label{subfig:hash-flow-b}}\\
  \subfloat[El \hcollector no conoce el hash]{\includegraphics[width = 3.5in]{figures/hash-reversal-3.png}\label{subfig:hash-flow-c}}\\
  \subfloat[Se anuncia a la red el conocimiento de un hash]{\includegraphics[width = 3.5in]{figures/hash-reversal-4.png}\label{subfig:hash-flow-d}}
  \caption{Flujo usual de mensajes para \vanilla}
