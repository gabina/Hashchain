...
Para evitar repeticiones, algunas definiciones de métodos que permanezcan sin cambios
...
\begin{figure}
  \subfloat[Estado inicial]{\includegraphics[width = 3in]{figures/vanilla-reflow-0.png}\label{subfig:vanilla-flow-a}}
  \subfloat[El cliente invoca \<add>(e)]{\includegraphics[width = 3in]{figures/vanilla-reflow-1.png}\label{subfig:vanilla-flow-b}}\\
  \vspace{0.5cm}
  \subfloat[$t_e$ se chequea contra \CheckTx]{\includegraphics[width = 3in]{figures/vanilla-reflow-2.png}\label{subfig:vanilla-flow-c}}
  \subfloat[Se consensúa el próximo bloque]{\includegraphics[width = 3in]{figures/vanilla-reflow-3.png}\label{subfig:vanilla-flow-d}}\\
  \vspace{0.5cm}
  \subfloat[Se envían los pedidos de la ABCI a la aplicación]{\includegraphics[width = 3in]{figures/vanilla-reflow-4.png}\label{subfig:vanilla-flow-e}}
  \subfloat[Se agrega la época $n$ a la \setchain]{\includegraphics[width = 3in]{figures/vanilla-reflow-5.png}\label{subfig:vanilla-flow-f}}
...
fue estampado, entonces, al agregar una nueva entrada a la base de datos, las claves podrían ser elementos, y los
...
\subsection{Prueba de membresía de elementos}\label{subsubsec:membership}
%
...
Los clientes pueden verificar si las pruebas de época son válidas generando el hash con el número
...
concluir que la época es correcta y que $e$ fue exitosamente insertado en la \setchain.
%
...
con un cliente y 4 servidores, de los cuales uno es bizantino (señalado en rojo).
%
...
%
Los servidores deciden en forma colaborativa los elementos pertenecientes a la época $n$.
...
\begin{figure}[!h]
  \centering
  \subfloat[\<add>(e)]{\includegraphics[width = 1.5in]{figures/epoch-proof-0.png}\label{subfig:epoch-proof-a}}
  \hspace{0.5in}
  \subfloat[Época $n$]{\includegraphics[width = 1.5in]{figures/epoch-proof-1.png}\label{subfig:epoch-proof-b}}\\
  \vspace{0.25in}
  \subfloat[Pruebas de época]{\includegraphics[width = 1.5in]{figures/epoch-proof-2.png}\label{subfig:epoch-proof-c}}
  \hspace{0.5in}
  \subfloat[\<get>]{\includegraphics[width = 1.5in]{figures/epoch-proof-3.png}\label{subfig:epoch-proof-d}}
...
%
Provee una función \texttt{Sign} que calcula una firma ECDSA.
%
...
%Este mecanismo de membresías de elementos a épocas está implementado en la definición de
%\<EndBlock>.
...
%presentada en~\ref{sec:setchain-properties}.
...
Para acercarnos al objetivo de \setchain, en esta sección proponemos
...
o bien una cantidad razonable de tiempo transcurrió desde que el primer elemento llegó. 
%
...
A su vez, un \collector bizantino también podría incorporar elementos inválidos en un lote.
%
...
Por esta razón no se presenta una definición para el método \<EndBlock>.
...
firmas por hash.
%
Además, cada vez que un hash es invertido, se chequea la corrección del lote original
...
Al igual que se hizo para \vanilla y \compresschain, en esta sub-sección se presentan los pseudocódigos
...
por lo tanto, sus elementos serán estampados con una época más temprana.
%
...
de front-running, dado que las transacciones observadas primero son probablemente las legítimas y
originales.
...
Sin embargo, la estrategia FSEC viene con algunos aspectos negativos no triviales que se presentan
a continuación.
