...
En este capítulo se exponen las pruebas de corrección de los algoritmos presentados para
...
para probar la corrección de ellas, será necesario recapitular las propiedades que la
...
\begin{property}[Tendermint-Global-Requests]\label{tendermint:global-delivery}
  Si un servidor correcto recibe una cadena de peticiones
...
\begin{proof}
  ...
  Por definición de \<add> en el algoritmo \ref{alg:api-vanilla}, la invocación
...
\begin{proof}
  ...
  % Si $e \in v.\HISTORY$, entonces, por la propiedad anterior, $e \in v.\THESET$ y la demostración es trivial.
  % Suponemos entonces $e \notin v.\HISTORY$ ($\clubsuit$).
  Por definición de \<add> en el algoritmo \ref{alg:api-brotli}, la invocación
  ...
  máximo, o bien una cantidad razonable de tiempo transcurrió desde que el primer
...
\begin{proof}
  ...
  Dado que $\SPH \geq f + 1$, podemos asegurar que al menos un servidor correcto (llamémoslo $w$) firmó $h$.
...
\begin{proof}
  ...
  Por definición de \<add> en el algoritmo \ref{alg:api-hashchain}, la invocación
...
\section{Conclusión}
Habiendo demostrado que \vanilla, \compresschain y \hashchain cumplen todas las propiedades
