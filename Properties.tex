...
% \begin{property}[Vanilla-Delivery]\label{tendermint:vanilla-delivery}
%   Sea $e$ un elemento válido y $v$ un servidor correcto.
%   Luego de una invocación
%   $v.\<add>(e)$ por parte de un cliente, eventualmente se tiene
%   $e \in v.\HISTORY$.
% \end{property}

% \begin{proof}
%   Sea $e$ un elemento válido y $v$ un servidor correcto.
%   Si $e \in v.\HISTORY$ luego la propiedad se cumple trivialmente.
%   Supongamos $e \notin v.\HISTORY$.
...
% No lo necesito si considero que add-before-get vale trivialmente
...
\begin{property}[Compresschain-Delivery]\label{tendermint:compresschain-delivery}
...
\begin{proof}
  ...
  % Considerando que la firma es válida por hipótesis,
...
\begin{proof}
  ...
  el servidor $v$ eventualmente recibirá una petición \<DeliverTx>($t$),
  en donde $t = e$.
  % por la propiedad~\ref{tendermint:vanilla-delivery} \emph{Vanilla-Delivery}
  % sabemos que
...
\begin{proof}
  ...
  en cuyo caso ya se cumple $e \in w.\THESET$ (ver lema \ref{lemma:vanilla-consistent-set}), o bien $e$ es un elemento nuevo.
  En este último caso,
  $e$ será añadido a
...
\begin{proof}
  ...
  Sea un elemento $e$ $\in v.\HISTORY(1)$ (por definición de \setchain, $\HISTORY$ empieza en la época 1). 
  ...
  peticiones \BeginBlock, [\DeliverTx, ...], \EndBlock, \Commit.
  Esto produjo que $e$ fuera añadido a $v.\HISTORY(1)$ y, posteriormente, que $v.\EPOCH$
  fuera incrementado de 0 a 1 como parte del proceso de la petición \EndBlock.
  ...
  Por lo tanto, de forma análoga, $w$ añadirá a $e$ a $w.\HISTORY(1)$, como se muestra en la línea
  \ref{line:abci-vanilla-history} en el Algoritmo~\ref{alg:abci-vanilla}.
  Posteriormente, $w.\EPOCH$ será incrementado de 0 a 1 como parte del proceso de la petición
  \EndBlock, como se muestra en la línea \ref{line:epoch-increment}.
  ...
  Luego, queda demostrada la propiedad para el caso base: $v.\HISTORY(n+1)=w.\HISTORY(n+1)$.
\end{proof}
...
\begin{proof}
  ...
  Si $e$ es nuevo, por la propiedad \textit{Compresschain-Delivery}~\ref{tendermint:compresschain-delivery} sabemos que
...
\begin{proof}
  ...
  es $e$.
  Si un servidor correcto recibe una petición \<DeliverTx>($t$), por la propiedad
  \emph{Tendermint-Global-Requests}~\ref{tendermint:global-delivery}
  todos los
  servidores correctos, y en particular $w$, reciben dicha petición \<DeliverTx>($t$).
...
\begin{proof}
  ...
  Se seguirá un razonamiento similar al presentado en la demostración del Lema ~\ref{proof:compresschain-get-global}.
...
\begin{proof}
  ...
  Aún más, por el lema \emph{Tendermint-Global-Requests}~\ref{tendermint:global-delivery} todos los servidores correctos recibirán $\<DeliverTx>(t)$.
...
\begin{proof}
  ...
  % $v$ en un momento determinado recibió una
  % petición \<DeliverTx>($t$), donde $t$ es un lote comprimido de elementos, y uno de esos
  % elementos es $e$. Como consecuencia de esta petición,
  % $e$ fue estampado con el número de época $i$.
  % Esto implica que el servidor decidió dicha transacción, puesto que la petición \<DeliverTx> se
