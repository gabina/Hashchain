...
\setchain~\cite{Capretto.2022.Setchain} es una estructura de datos concurrente y distribuída que implementa
conjuntos distribuídos que solo crecen, con barreras de sincronización (llamadas épocas).
%
...
%
Luego, una \setchain se define como una estructura de datos distribuída donde cada de nodo servidor
(correcto) mantiene:
\begin{itemize}
...
Se usa notación de punto para invocar estas operaciones. De este modo:
\begin{itemize}
  \item $v.\<add>(e)$: solicita agregar el elemento $e$ a $\THESET$ (a la versión de $\THESET$ que $v$ mantiene).
...
%
En este trabajo se asume que las barreras de sincronización se lanzan periódicamente.
%
Sin embargo, en la interfaz original de \setchain, un nodo servidor $v$ también provee una operación para
forzar un incremento de época, $v.\<epocinc>(h)$.
%
